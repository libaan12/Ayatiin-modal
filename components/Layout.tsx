...
export const Layout = ({ children, user, role }: any) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      <div 
        className={`fixed inset-0 z-20 bg-black bg-opacity-50 lg:hidden transition-opacity duration-300 ${sidebarOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} 
        onClick={() => setSidebarOpen(false)}
      ></div>

      {/* Sidebar */}
      <aside 
        className={`fixed inset-y-0 left-0 z-30 w-64 bg-primary-50 text-slate-800 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static flex flex-col shadow-xl border-r border-primary-100 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex items-center justify-center h-20 bg-primary-600 shadow-md px-4">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Header - Solid Green */}
        <header className="flex justify-between items-center h-16 bg-primary-600 shadow-md px-6 z-10 sticky top-0">
          {/* Hamburger Menu - Visible only on mobile/tablet */}
          <button className="lg:hidden text-primary-100 hover:text-white focus:outline-none p-2 rounded-md hover:bg-primary-700" onClick={() => setSidebarOpen(true)}>
